...
def evaluate_line(line, player):
    ...
    if player_stones == 5: return 100000 # 勝ち
    if player_stones == 4: return 1000   # リーチ
...
def get_score(board_grid, player):
    opponent = PLAYER if player == AI else AI
    # 攻撃重視にしすぎるとすぐ勝ってしまうので、バランスはそのまま
...
def pick_move_reception_smart(board, ai_player, ai_turn_count):
    ...
    if winning_moves:
        miss_chance = min(ai_turn_count * 0.01, 1.0)
        if random.random() > miss_chance:
            print(f"AI: 勝ち手が見えたので、容赦なく勝ちに行きます (見逃し率: {int(miss_chance*100)}%)")
            return winning_moves[0][0]
        else:
            print(f"AI: 勝ち手がありましたが、疲れで見逃しました... (見逃し率: {int(miss_chance*100)}%)")
            scored_moves = [m for m in scored_moves if m[1] < 90000]

    # --- 4. [重み付け選択] (マイルド調整) ---
    if not scored_moves: return random.choice(candidates)
    if len(scored_moves) == 1: return scored_moves[0][0]
    
    target_indices = []
    weights = []
    
    # バランス調整: 2位の確率を下げ、3,4位を上げることで「強すぎず弱すぎず」を狙う
    # 2位 (Index 1)
    if len(scored_moves) > 1: 
        target_indices.append(1)
        weights.append(30) # 前回40 -> 30 (強さを抑える)
    # 3位 (Index 2)
    if len(scored_moves) > 2: 
        target_indices.append(2)
        weights.append(40) # 前回40 -> 40 (据え置き。ここがメイン)
    # 4位 (Index 3)
    if len(scored_moves) > 3: 
        target_indices.append(3)
        weights.append(20) # 前回15 -> 20 (少し弱くする頻度アップ)
    # 5位 (Index 4)
    if len(scored_moves) > 4: 
        target_indices.append(4)
        weights.append(10) # 前回 5 -> 10 (たまにポカする頻度アップ)

    if not target_indices: return scored_moves[0][0]
    chosen_idx = random.choices(target_indices, weights=weights, k=1)[0]
    
    print(f"AI: 上位候補(2~5位)の中から、重み付けで {chosen_idx + 1}番目の手を選びました")
...
def main():
    ...
    while True:
        ...
        if key == ord('s') and not game_started:
            ...
            # Level 2 calling convention: (board, ai_player, ai_turn_count)
