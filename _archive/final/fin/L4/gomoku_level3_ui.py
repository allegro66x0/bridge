...
def get_score(board_grid, player):
    ...
    # ★変更点1: 攻撃重視 (自分の得点倍率アップ、相手の妨害倍率ダウン)
    # これにより「守るより攻める」性格になり、ジリ貧を防ぐ
    my_score = count_patterns_for_player(board_grid, player)
    op_score = count_patterns_for_player(board_grid, opponent)
    
    return my_score * 1.5 - op_score * 1.0

# ===============================================================
# ★Level 3: 師匠モード（攻撃強化・人間味調整版）
# ===============================================================
def pick_move_reception_smart(board, ai_player, ai_turn_count):
    ...
    for (r, c) in candidates:
        ...
        # ★変更点2: 序盤(3手目以内)は中央重視
        # 中央からの距離を計算し、遠い場所の評価値を大幅に下げる
        if ai_turn_count <= 3:
            dist = max(abs(r - center_idx), abs(c - center_idx))
            if dist > 3: # 中央7x7マスの外側なら
                score -= 500 # 評価を下げる
    ...
    # --- 1. [勝利確定チェック] (最優先) ---
    # 相手のリーチがあっても、自分が勝てるなら勝つ！
    winning_moves = [m for m in scored_moves if m[1] >= 90000]
    if winning_moves:
        # ★変更点4: 見逃し率は 0.5% ずつ上昇
        miss_chance = min(ai_turn_count * 0.005, 1.0)
        
        if random.random() > miss_chance:
            print(f"AI: 勝ち手を発見！攻撃優先で勝ちに行きます (見逃し率: {miss_chance:.1%})")
            return winning_moves[0][0]
        else:
            print(f"AI: 勝ち手がありましたが、疲れで見逃しました... (見逃し率: {miss_chance:.1%})")
            # 勝ち手を除外して続行
            scored_moves = [m for m in scored_moves if m[1] < 90000]

    # --- 2. [防衛] プレイヤーの攻撃を防ぐ ---
    # 自分が勝てない場合のみ、防衛判定を行う
    
    critical_defenses = [] # 負け確定(5連される)を防ぐ手
    warning_defenses = []  # リーチ(4連される)を防ぐ手
    ...
    for (r, c) in candidates:
        ...
        if max_len >= 5: critical_defenses.append((r, c))
        elif max_len >= 4: warning_defenses.append((r, c))

    if critical_defenses:
        print(f"AI: プレイヤーの勝利(5連)を検知！全力で阻止します。")
        return random.choice(critical_defenses)
    
    if warning_defenses:
        print(f"AI: プレイヤーのリーチ(4連)を検知！防衛します。")
        return random.choice(warning_defenses)

    # --- 3. [重み付け選択] (Level 3仕様) ---
    # 防衛の必要がない場合、攻撃的な手を選ぶ
    if not scored_moves: return random.choice(candidates)
    
    # ★変更点3: 上位3つから選び、2位を重視する
    target_indices = []
    weights = []
    ...
    # 4位以下は選ばない (weights list length check)
    if len(weights) == 0:
         return scored_moves[0][0]

    chosen_idx = random.choices(target_indices, weights=weights, k=1)[0]
    
    print(f"AI: 上位候補(1~3位)の中から、重み付けで {chosen_idx + 1}番目の手を選びました")
    return scored_moves[chosen_idx][0]
...
def main():
    ...
    while True:
        ...
        if key == ord('s') and not game_started:
            ...
            # Level 3 calling convention: (board, ai_player, ai_turn_count)
            move = pick_move_reception_smart(current_board, AI, ai_turn_count)
