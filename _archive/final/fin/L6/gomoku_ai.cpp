...
std::vector<int> count_pattern_counts(const Board& board, int player) {
    // counts[0]=5連, counts[1]=4連, counts[2]=3連, counts[3]=2連
    std::vector<int> counts = {0, 0, 0, 0};
    
    // 重複カウントを防ぐため、方向ごとにスキャンするシンプルな実装
    // ただし、Python版の evaluate_line の挙動（部分列の全探索）に合わせる必要がある
    // Python版: line[i:i+5] の窓をスライドさせている
    
    // 横・縦・斜めのすべての5マスの並びをチェック
    auto evaluate_window = [&](const std::vector<int>& line) {
        int player_stones = 0;
        int opponent = (player == AI_PLAYER) ? HUMAN_PLAYER : AI_PLAYER;
        
        for (int cell : line) {
            if (cell == opponent) return; // 相手の石があれば価値なし
            if (cell == player) player_stones++;
        }
        
        if (player_stones == 5) counts[0]++;
        else if (player_stones == 4) counts[1]++;
        else if (player_stones == 3) counts[2]++;
        else if (player_stones == 2) counts[3]++;
    };
    ...
    for (int r = 0; r < BOARD_SIZE; ++r) {
        for (int c = 0; c <= BOARD_SIZE - 5; ++c) {
            std::vector<int> line;
            for (int i = 0; i < 5; ++i) line.push_back(board[r][c+i]);
            evaluate_window(line);
        }
    }
    // 縦
    for (int c = 0; c < BOARD_SIZE; ++c) {
        for (int r = 0; r <= BOARD_SIZE - 5; ++r) {
            std::vector<int> line;
            for (int i = 0; i < 5; ++i) line.push_back(board[r+i][c]);
            evaluate_window(line);
        }
    }
    // 斜め (右下)
    for (int r = 0; r <= BOARD_SIZE - 5; ++r) {
        for (int c = 0; c <= BOARD_SIZE - 5; ++c) {
            std::vector<int> line;
            for (int i = 0; i < 5; ++i) line.push_back(board[r+i][c+i]);
            evaluate_window(line);
        }
    }
    // 斜め (左下)
